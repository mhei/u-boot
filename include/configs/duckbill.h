...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"netboot=echo Booting from net ...; " \
		...
		"else " \
			"bootz; " \
		"fi\0" \
	"find_bootsys=echo Trying to detect boot system ...; " \
		"if test -n ${bootsys}; then " \
			"if test -n ${checksys}; then " \
				"if test ${checksys} = 0; then " \
					...
					"if test ${bootsys} = 1; then " \
						"echo Booting system 2; " \
						"setenv bootsys 2; " \
						"run loadimage mmcboot; " \
					"else " \
						"echo Booting system 1; " \
						"setenv bootsys 1; " \
						"run loadimage mmcboot; " \
					"fi; " \
				"else " \
			...
			"fi; " \
			"if test ${bootsys} = 1; then " \
				"echo Booting system 1 && run loadimage mmcboot || " \
				"echo Booting backup system 2 && setenv bootsys 2 && " \
					"run loadimage mmcboot; " \
			"else " \
				"if test ${bootsys} = 2; then " \
					"echo Booting system 2 && run loadimage mmcboot || " \
					"echo Booting backup system 1 && setenv bootsys 1 && " \
						"run loadimage mmcboot; " \
				"else " \
					"echo Warning: bootsys does not contain 1 or 2, " \
						"trying to boot system 1; " \
					"setenv bootsys 1; " \
					"run loadimage mmcboot; " \
				"fi; " \
			"fi; " \
		"else " \
			"echo Error: bootsys is not set, falling back to netboot.; " \
			"run netboot; " \
		"fi\0"
